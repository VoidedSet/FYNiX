...
| 1.8 | `nextID` is not reused after deletions (ID fragmentation) | 🟡 Improvement | **Done** |
| **2** | **Physics System** | | |
...
| 2.6 | Scale applied via `setLocalScaling` doesn't recompute inertia | 🟠 Caveat | **Done** |
| 2.7 | `setGravity(int)` truncates fractional float gravity values | 🟠 Caveat | **Done** |
...
| 4.3 | Texture bind/unbind slot mismatch during async asset loads | 🟠 Caveat | **Done** |
| 4.4 | Uniform locations are re-queried via `glGetUniformLocation` on every uniform set | 🟡 Improvement | **Done** |
| 4.5 | `std::string` heap allocations occur inside hot render path loop | 🟡 Improvement | **Done** |
| **5** | **Scene Save / Load** | | |
